...
add_section_heading(doc, "三、赛程安排：分阶段冲刺，直通 WAIC")

add_styled_paragraph(
    doc,
    "本次大赛为期近四周（6月18日 - 7月12日），节奏紧凑，期间设置三个阶段里程碑，"
    "每个阶段评选一支代表团队获得 WAIC 门票，最终进行独立总评：",
    font_size=11, color=DARK_TEXT, space_after=8
)

add_info_table(doc, [
    ["阶段", "时间", "内容"],
    ["上线 / 报名开始", "6月18日", "在魔搭社区完成身份绑定，加入官方交流群，即刻开发"],
    ["阶段 1：选题登记", "6月18日 - 6月24日", "登记选题并完成可行性说明；阶段结束评选「最佳选题潜力奖」，发放 1 张 WAIC 门票"],
    ["阶段 2：MVP 开发", "6月25日 - 7月1日", "提交可运行 MVP 原型；阶段结束评选「最佳 MVP 原型奖」，发放 1 张 WAIC 门票"],
    ["阶段 3：社区共建", "7月2日 - 7月8日", "鼓励社区试用、反馈与传播；阶段结束评选「社区影响力奖」，发放 1 张 WAIC 门票"],
    ["最终提交截止", "7月12日 23:59", "将完整作品发布至魔搭社区，并填写最终提交表单"],
    ["集中评审", "7月13日 - 7月14日", "独立专家六维评分制评审"],
    ["结果公示与颁奖", "7月15日", "公布最终获奖名单，发放云资源/API 额度"],
    ["WAIC 赛果展示", "7月17日 - 7月20日", "获奖代表于上海 WAIC 大会现场做赛果展示"],
])
...
add_section_heading(doc, "四、评审规则")

add_styled_paragraph(
    doc,
    "本次评审分为「阶段奖评审」和「最终总评」两部分，确保不同类型的优秀项目都能脱颖而出：",
    font_size=11, color=DARK_TEXT, space_after=8
)

add_styled_paragraph(doc, "阶段奖评审（各阶段独立评审，产生 WAIC 门票获得者）",
                     font_size=11, bold=True, color=DARK_TEXT, space_after=4)

add_bullet(doc, "重点看选题是否真实、有价值、可在短周期内落地。", bold_prefix="阶段 1（选题潜力）：")
add_bullet(doc, "奖励最早把核心能力跑起来的队伍，必须能演示。", bold_prefix="阶段 2（MVP 原型）：")
add_bullet(doc, "综合有效调用、收藏、评论质量、传播质量，异常流量剔除。", bold_prefix="阶段 3（社区影响力）：")

add_styled_paragraph(doc, "最终总评（7/13-7/14，六维评分制）",
                     font_size=11, bold=True, color=DARK_TEXT, space_before=10, space_after=4)

add_styled_paragraph(
    doc,
    "最终总评采用独立专家六维评分制，社区反馈质量仅占 10%，不直接主导排名：",
    font_size=10.5, color=DARK_TEXT, space_after=6
)

add_info_table(doc, [
    ["评分维度", "权重", "说明"],
    ["技术实现与可运行性", "25%", "代码结构清晰、核心功能能跑、部署稳定"],
    ["医疗场景价值与问题定义", "20%", "场景真实、目标用户明确、解决方案有意义"],
    ["MCP/Skill 完成度与平台适配", "20%", "是否符合魔搭社区提交、运行和展示要求"],
    ["开源质量与文档可复现性", "15%", "公开仓库、License、README、部署步骤、示例输入输出"],
    ["安全合规与风险控制", "10%", "不使用真实患者数据，不承诺诊断，明确局限性"],
    ["社区反馈质量", "10%", "真实用户反馈、Issue、有效讨论，而非单纯调用数"],
])
...
add_styled_paragraph(
    doc,
    "（注：奖励方案中云资源/API 额度的最终金额与形式以与魔搭社区协商结果为准）",
    font_size=9, color=GRAY_TEXT, space_after=8
)

# ── 阶段奖 ──
add_styled_paragraph(doc, "阶段奖（WAIC 门票直通）",
                     font_size=12, bold=True, color=DARK_TEXT, space_after=6)

add_styled_paragraph(
    doc,
    "每个阶段产生 1 支代表团队，获奖团队将获得 1 张 WAIC 门票（7月17-20日，上海），"
    "并接受官方开发者采访与项目介绍：",
    font_size=10.5, color=DARK_TEXT, space_after=6
)
...
add_info_table(doc, [
    ["阶段", "奖项", "奖励"],
    ["6/18–6/24", "🎯 最佳选题潜力奖", "1 张 WAIC 门票 + 开发者采访"],
    ["6/25–7/1", "⚡ 最佳 MVP 原型奖", "1 张 WAIC 门票 + 开发者采访"],
    ["7/2–7/8", "🌟 社区影响力奖", "1 张 WAIC 门票 + 开发者采访"],
])

doc.add_paragraph()

# ── 最终奖 ──
add_styled_paragraph(doc, "最终总评奖项（7月15日公布）",
                     font_size=12, bold=True, color=DARK_TEXT, space_after=6)

add_info_table(doc, [
    ["奖项", "名额", "奖励内容"],
    ["🏆 一等奖", "Top 3（3名）", "1000 RMB 云资源或 API 额度"],
...
# ── 阶段奖与公平性规则 ──
add_styled_paragraph(doc, "阶段奖与公平性规则",
                     font_size=12, bold=True, color=DARK_TEXT, space_after=6)

add_bullet(doc, "每个获奖团队获得 1 张 WAIC 门票，由团队自行指定 1 名代表领取。", bold_prefix="门票归属：")
add_bullet(doc, "每支队伍在整个比赛期间最多获得 1 次 WAIC 门票类奖励。", bold_prefix="不可重复：")
add_bullet(doc, "已获票团队仍可继续参与后续阶段展示和最终总评，但不再参与后续 WAIC 门票类阶段奖评选。",
           bold_prefix="后续参赛：")
add_bullet(doc, "若某阶段最高分团队已获票，门票顺延至该阶段下一支未获票且符合资格的队伍。",
           bold_prefix="顺延机制：")
add_bullet(doc, "阶段获奖后 24-48 小时内完成 5 个问题的轻量采访，用于公众号/魔搭社区/活动页宣传。",
           bold_prefix="采访机制：")
add_bullet(doc, "每人只能加入一队；同一核心成员、同一代码库的报名视为同一参赛主体。", bold_prefix="队伍归属：")
add_bullet(doc, "允许历史项目参赛，但必须披露赛前已有部分，评审重点看比赛期间新增贡献。",
           bold_prefix="历史项目：")
add_bullet(doc, "异常流量（刷量、互刷、机器人行为）将被剔除，情节严重者取消评奖资格。", bold_prefix="反作弊：")
...
faq_list = [
    ...
    ("Q6：WAIC 门票如何获得？",
     "门票分三个阶段提前发放：6/24 评选「最佳选题潜力奖」、7/1 评选「最佳 MVP 原型奖」、"
     "7/8 评选「社区影响力奖」，每阶段 1 张。已获票团队不再参与后续门票评选，门票顺延至下一支符合条件的队伍。"),
    ("Q7：奖励如何发放？",
     "WAIC 门票在阶段奖公布时同步发放，以便提前安排行程。云资源或 API 额度将在7月15日结果公示后统一发放。"),
    ("Q8：如何加入交流群？",
